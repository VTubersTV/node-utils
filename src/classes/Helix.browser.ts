...
/**
 * Helix - A high-performance distributed unique ID and token generator (Browser Version)
 *
 * Features:
...
export class Helix {
    ...
    constructor(options: { workerId?: number; tokenSecret?: string } = {}) {
        ...
        // Allow manual worker ID override, otherwise generate a random one
    ...
    public generateToken(data: unknown): string {
        ...
        const payload = btoa(jsonData).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
    ...
    public verifyToken<T = unknown>(token: string): T {
        ...
        try {
            // Decode payload
            const jsonData = atob(payload.replace(/-/g, '+').replace(/_/g, '/'));
    ...
    /**
     * Signs a payload using a simple hash function for browser compatibility
     * @param payload The payload to sign
     * @returns The base64url encoded signature
     */
    private signPayload(payload: string): string {
        // Simple hash function for browser compatibility
        const hash = Array.from(payload + this.tokenSecret).reduce((acc, char) => {
